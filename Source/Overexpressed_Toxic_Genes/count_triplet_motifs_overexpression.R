### Script: count_triplet_motifs_overexpression.R
...
observed_a_and_b_overexpressed_toxic_gene_counts <- "observed_a_and_b_overexpressed_toxic_gene_counts.tab"
observed_c_overexpressed_toxic_gene_counts 	   <- "observed_c_overexpressed_toxic_gene_counts.tab"

randomized_count_a_and_b_table_file 	<- "randomized_results_table_overexpressed_toxic_gene_counts_a_and_b.tab"
randomized_count_c_table_file 		<- "randomized_results_table_overexpressed_toxic_gene_counts_c.tab"

output_full_results_table_count_a_and_b	<- "full_results_table_overexpressed_toxic_gene_counts_a_and_b.tab"		  
output_full_results_table_count_c  	<- "full_results_table_overexpressed_toxic_gene_counts_c.tab"

## Tidy up overexpression table 
overexpressed_toxic_genes <- as.data.frame( overexpressed_toxic_genes[, "oln_id"])
colnames( overexpressed_toxic_genes) <- "oln_id"
...
count_overexpressed_toxic_genes_in_motifs <- count_triplet_motifs_against_gene_list( triplet_motifs_costanzo, overexpressed_toxic_genes)


results_count_a_and_b <- transpose_count_triplet_motifs_results(count_overexpressed_toxic_genes_in_motifs[,c("type_ac", 
																									"type_bc", "count_a_and_b")], 
																				  "type_ac", "type_bc", "count_a_and_b", "motif_type") 


results_count_c <- transpose_count_triplet_motifs_results(count_overexpressed_toxic_genes_in_motifs[,c("type_ac", 
																							 "type_bc", "count_c")], 
...
# 																   num_iterations=1, count_triplet_motifs_against_gene_list,
