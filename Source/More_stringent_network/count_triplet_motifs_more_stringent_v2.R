...
if ( length(options) >= 4 ) {
	quantile_chosen <- as.numeric(options[4])
	print( paste( "quantile_chosen = " , quantile_chosen ))
	
	if ( !( quantile_chosen %in%   seq(0.05, 1.00, 0.05)*100) )   {
		warning( paste( "incorrect quantile chosen", quantile_chosen) )
	}
}

quantile_chosen <- paste( quantile_chosen, "%", sep="")
...
quantile_threshold <- quantile( filtered_costanzo_stringent[, "genetic_interaction_score"], probs=seq(0.05, 1.00, 0.05))

fitness_threshold <- quantile_threshold[[quantile_chosen]]

#### filter the negative genetic interaction networks base on the fitness score threshold
filtered_costanzo_stringent <- dplyr::filter( filtered_costanzo_stringent,  genetic_interaction_score <= fitness_threshold) 
