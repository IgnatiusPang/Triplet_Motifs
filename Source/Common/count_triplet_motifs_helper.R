...
##  	* sbi_interactome must have these columns: oln_id_a, oln_id_b
##  	* rewired_tf_network must have these columns: regulator_oln_id, target_oln_id
##      * use_rewired_interactions: The rewired integrated biological network table will be passed onto another function, defined in 'FUNC'
##      * is_rewire_genetic_interactions: Whether to rewire the genetic interaction network.
...
run_one_randomized_trial_compare_with_dataset <- function (run_number, filtered_costanzo_stringent, kinase_network, sbi_interactome, tf_network, 
	...
	rewired_filtered_costanzo_stringent <- filtered_costanzo_stringent
	
	if ( is_rewire_genetic_interactions == TRUE) {
		
		rewired_filtered_costanzo_stringent <- rewire_genetic_interactions_table(
														filtered_costanzo_stringent, "oln_id_a",
														"oln_id_b", "genetic_interaction_score", 
														directed=FALSE, num_iterations=num_iterations,
														min_num_neighbours=min_num_neighbours, 
														max_num_tries=max_num_tries, mode=mode)
	}
...
##################################################################################################################
### Convert ID to attribute
create_id_to_attribute_hash <- function(  key, attribute) {
	
	key <- as.character( as.vector(key))
	attribute <- as.vector(attribute)
	
	hash <- new.env(hash = TRUE, parent = parent.frame())
	
	if ( length(key) != length(attribute))  {
		warning('Length of key != Length of attribute list.')
		return(1);
	}
	
	for ( i in 1:length(key) )  {
		assign( key[i] , attribute[i] , envir = hash)                       
	}
...
##################################################################################################################
#### Function to convert any Key to Attribute, return NA if key does not exists
